      ...
      {sources.map((source) => {
        ...
        return (
          <article className={selectedSourceId === source.source_id ? "collection-card selected" : "collection-card"} key={source.source_id}>
            ...
            <p className="summary">{source.source_type} · {ui.common.updated} {formatDate(source.updated_at ?? source.last_seen_at)}</p>
            <div className="tag-row">{source.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
            <dl className="property-grid compact">
              <div><dt>{ui.common.nodes}</dt><dd>{sourceNodes.length}</dd></div>
              <div><dt>{ui.common.edges}</dt><dd>{sourceEdges.length}</dd></div>
              <div><dt>{ui.common.report}</dt><dd>{report ? ui.common.yes : ui.common.no}</dd></div>
            </dl>
            <DigestPreview digest={digest} />
            <div className="source-sections">
              <section>
                <div className="section-title-row"><h3>{ui.sourceDetail.generatedNodes}</h3><small>{sourceNodes.length}</small></div>
              ...
              <section>
                <div className="section-title-row"><h3>{ui.sourceDetail.relatedEdges}</h3><small>{sourceEdges.length}</small></div>
                <div className="compact-list">
            ...
            {report ? <button className="secondary-action" onClick={() => onOpenReport(source.source_id)}>{ui.sourceDetail.openReport}</button> : null}
          </article>
        );
      })}
      {sources.length === 0 ? <p className="empty-state">{ui.sourceDetail.noSource}</p> : null}
    </div>
  );
}
...
function DigestPreview({ digest }: { digest?: ThoughtDigestDoc }) {
  const ui = useUiText();
  if (!digest) return <p className="empty-state">{ui.sourceDetail.noDigest}</p>;
  return (
    <section className="digest-preview">
      <h3>{ui.sourceDetail.digest}</h3>
      <p className="summary">{digest.summary}</p>
      <small>{digest.item_count} {ui.sourceDetail.digestItems} · {formatDate(digest.created_at)}</small>
    </section>
  );
}
